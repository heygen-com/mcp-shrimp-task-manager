...
import { parseCommentsForTasks } from "../../utils/jiraCommentTaskParser.js";
import { 
  JiraCommentService,
  type JiraComment,
  type JiraCommentCreateRequest, 
  type JiraCommentUpdateRequest
} from "./jiraCommentService.js";
...
// Create JiraCommentService instance
const jiraCommentService = new JiraCommentService();
...
// Additional interfaces for task handling
interface JiraImage {
  id: string;
  base64Data: string;
  contentType: string;
...
interface JiraTaskWithImages extends CommentTask {
  downloadedImages?: JiraImage[];
  hasImages: boolean;
  isOcrReady: boolean;
  images: JiraImage[];
  uiHints?: {
    ...
    case "get_comment_tasks":
      if (input.domain === "ticket") {
        ...
        try {
          ...
          const simplifiedTasks = parseResult.tasks.map((task: CommentTask) => {
            const enhancedTask = task as EnhancedCommentTask;
            
            // Only include successful downloads
            const successfulImages = enhancedTask.downloadedImages?.filter((img: JiraImage) => img.base64Data && !img.error) || [];
          ...
          const tasksWithImages = simplifiedTasks.filter((task: JiraTaskWithImages) => task.hasImages);
          if (tasksWithImages.length > 0) {
            markdown += `## 📷 Tasks with Downloaded Images (${tasksWithImages.length})\n\n`;
            tasksWithImages.forEach((task: JiraTaskWithImages, index: number) => {
              markdown += `### ${index + 1}. ${task.text}\n`;
              markdown += `**Images:** ${task.images.length} successfully downloaded\n`;
              if (task.uiHints) {
                const { pages, components } = processTaskHints(task);
                markdown += `**UI Context:** Pages: ${pages.join(', ')} | Components: ${components.join(', ')}\n`;
              }
              markdown += `\n`;
            });
          }
          ...
          return {
            ...
            json: {
              ...
              summary: {
                ...
                totalImagesDownloaded: simplifiedTasks.reduce((total: number, task: JiraTaskWithImages) => total + task.images.length, 0),
                ocrReadyTasks: simplifiedTasks.filter((task: JiraTaskWithImages) => task.isOcrReady).length,
                detectedPages: [...new Set(simplifiedTasks.flatMap((task: JiraTaskWithImages) => task.uiHints?.pages || []))],
                detectedComponents: [...new Set(simplifiedTasks.flatMap((task: JiraTaskWithImages) => task.uiHints?.components || []))]
              }
            },
            url: `${getJiraEnv().baseUrl}/browse/${issueKey}`,
          };
...
// Update the task processing code to handle undefined uiHints
function processTaskHints(task: JiraTaskWithImages) {
  return {
    pages: task.uiHints?.pages || [],
    components: task.uiHints?.components || []
  };
}