    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      return {
        tools: [
          ...
          { name: "jira", description: "Manages JIRA items. Actions: create (create tickets/epics), update (modify items), read (get ticket details), find (search items), list (show items), delete (delete tickets), sync (sync with JIRA), verify_credentials (check auth), find_user (search users), history (get ticket changelog), get_comment_tasks (extract tasks from ticket comments), update_comment_task (mark comment tasks complete), create_comment (add new comment), read_comments (get comments), update_comment (modify comment), delete_comment (remove comment), list_comments (search/filter comments). Domain: ticket (for all ticket and comment operations), project, component, migration, user. Use action parameter.", inputSchema: zodToJsonSchema(JiraToolSchema) },
